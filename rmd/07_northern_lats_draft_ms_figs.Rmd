...
p_density_hist_cc_sst_2006_2007_2023 <- df %>%    
    ...
    labs(x = 'SST (°C)', y= "Density",
         subtitle = "September Turtle-SST (°C) Preference") +
    theme_bw() +
     # theme_minimal()+   
...
p_density_hist_cc_chl_2006_2007_2023 <- df %>%    
    ...
    labs(x = 'Chla (mg/m^3)', y= "Density",
         subtitle = "September Turtle-Chla (°C) Preference") +
    theme_bw() +
     # theme_minimal()+   
    # theme(legend.position = c(0.90,0.85)) +
    theme(legend.justification = "top") +
...
add_contour_lines <- function(df, contours, col='black'){
    list(
        ...
        colour = col, linewidth = 1.25, breaks = c(contours), show.legend = T)
    )
}
...
```{r 07-03rmd-ras-sept-means-to-df}

## NEED THIS CELL??

# sst_ras_mean_sept_2006_2007
```
...
p_caption <- p +
    ...
    labs(caption = "\nAverage 18C SST Isotherm Location:\n1985 - 1995 (light gray); 1990 - 1994 (medium gray); 2020 - 2023 (black)"
         ) 

p_caption +  annotate("text", x=-165, y=45, label= "September") +
    annotate("text", x=-167, y=27.5, label= "March")
