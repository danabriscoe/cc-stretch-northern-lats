...
# save
ggsave(filename = 'plot_avg_turtle_lats_by_week.png', plot = p_avg_turtle_lats_by_week + labs(subtitle=""), 
       # height=9,
       width=11, bg='white',
       path = '../figs/'
       )
...
```{r 02-hist-daily-hist-cohort1-sept-by-lat-bins}

df_165W %>%
    filter(month == 9) %>%
    filter(year != 2023) %>%
    group_by(group, day) %>% # only interested in avg daily lat during sept, from 97-22, so don't need id/names
      summarize(
        avg = mean(lat, na.rm=TRUE)) %>% #,
...
ggplot(aes(avg)) +    
    geom_histogram(binwidth = 0.05, fill = "steelblue", colour = "navy", alpha = 0.6) + 
