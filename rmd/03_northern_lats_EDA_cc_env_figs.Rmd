...
```{r 03-plot-ssta-mean-no-cc-tracks}

p_ssta_mean_no_cc <- ggplot() +
    # add ssta layer
...
p_ssta_mean_no_cc_or_tcms <- p_ssta_mean_no_cc #+ add_tcms_box()
# p_ssta_mean_no_cc_with_tcms <- p_ssta_mean_no_cc + add_tcms_box()


# save
ggsave(filename = str_c('plot_ssta_mean_no_cc_or_tcms_w_contour.png'), plot = p_ssta_mean_no_cc_or_tcms + resize_cbar(24) +
           labs(caption = ''), 
       # height=9,
       width=13, bg='white',
       path = '../figs/'
...
# ggsave(filename = str_c('plot_ssta_mean_no_cc_with_tcms_w_contour.png'), plot = p_ssta_mean_no_cc_with_tcms + 
...
ras_sept_1985_1995 <- sst_ras_stack[[i_hist_sept_1985_1995]]

# 1990-1994
i_hist_sept_1990_1994 <- dte[(m == "Sep" & y >= "1990" & y <= "1994")] %>% gsub(" ", ".", .)
ras_sept_1990_1994 <- sst_ras_stack[[i_hist_sept_1990_1994]]

# 2020-2023
i_hist_sept_2020_2023 <- dte[(m == "Sep" & y >= "2020" & y <= "2023")] %>% gsub(" ", ".", .)
ras_sept_2020_2023 <- sst_ras_stack[[i_hist_sept_2020_2023]]
...
# 3006-2007 Sept subset
chl_mday_ras_stack_sept_2006_2007 <- subset(chl_mday_ras_stack_sept, c(4:5))
names(chl_mday_ras_stack_sept_2006_2007) <- as.character(sept_fdates)[4:5]  # mon-yr abbreb
...
## PLOT (w/o u-turn tags)
p_sept_mean_chla_aug_sst_2006_2007_2023_rm_uturns <- ggplot(chl_mday_ras_mean_sept_2006_2007_df %>% drop_na(), aes(lon, lat)) +
  # geom_raster(aes(x = lon, y = lat, fill = cut(val, zCuts), interpolate = TRUE, na.rm = TRUE)) +
     geom_tile(aes(x = lon, y = lat, fill = cut(val, zCuts), interpolate = TRUE, na.rm = TRUE)) +
  scale_fill_manual("Mean Chla", na.translate = F,
                    values = rev(myPallette), na.value="transparent",
    ...
    guides(fill = guide_legend(title = 'Mean Sept\nChla (mg/m^3)')) + 
     ...
     add_turtle_pts(df=mean_lats_cohort1, col="2023", size=3) +
       ...
       scale_color_manual("Mean Sept\nTurtle Location", 
        ...
        labs(subtitle = "Monthly Mean Sept Chl-a (2006-2007) and Turtle Location (2006-2007, 2023)",
         caption = str_c("Isotherms: Sept 18°C (2006-2007, gray line), Sept 19.5°C (2006-2007, black line)\n 
         August 18°C (2006-2007, dotted gray line), August 19.5°C (2006-2007, dotted black line)\n
                         Sept 18°C (2023, pink line)")) + 
         xlab("\nLongitude") +
         ylab("Latitude\n")
